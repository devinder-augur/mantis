...
func handlePushEventApplyAfterMerge(gh utils.GithubClientProvider, payload *github.PushEvent) error {
	...
	if strings.HasSuffix(ref, defaultBranch) {
		...
		// create 2 jobspecs (mantis plan, mantis apply) using commitID
		// TODO: find a way to get issue number from github api PushEvent
		// TODO: find a way to set the right PR branch
		issueNumber := 14
		planJobs, err := dg_github.CreateJobsForProjects(impactedProjects, "mantis plan", "push", repoFullName, requestedBy, config.Workflows, &issueNumber, &commitId, defaultBranch, defaultBranch)
		if err != nil {
			log.Printf("Error creating jobs: %v", err)
			return fmt.Errorf("error creating jobs")
		}

		applyJobs, err := dg_github.CreateJobsForProjects(impactedProjects, "mantis apply", "push", repoFullName, requestedBy, config.Workflows, &issueNumber, &commitId, defaultBranch, defaultBranch)
		if err != nil {
			log.Printf("Error creating jobs: %v", err)
			return fmt.Errorf("error creating jobs")
		}
