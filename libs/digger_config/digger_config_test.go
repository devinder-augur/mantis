...
func TestDiggerConfigWhenMultipleConfigExist(t *testing.T) {
	...
	_, err := os.Create(path.Join(tempDir, "mantis.yaml"))
	if err != nil {
		t.Fatal(err)
	}

	_, err = os.Create(path.Join(tempDir, "mantis.yml"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestDiggerConfigWhenCustomFileName(t *testing.T) {
	...
	assert.Nil(t, err)
	assert.Equal(t, configPath, path.Join(tempDir, "mantis-custom.yml"))
...
- name: dev
  ...
  workspace: default
`
	deleteFile := createFile(path.Join(tempDir, "mantis.yml"), diggerCfg)
	defer deleteFile()
...
workflows:
  myworkflow:
    ...
    workflow_configuration:
      ...
      on_commit_to_default: [mantis apply]
    env_vars:
      state:
      - name: TF_VAR_state
...
- name: dev
  ...
  terragrunt: true
`
	defer createFile(path.Join(tempDir, "mantis.yml"), diggerCfg)()
...
`
	...
	err = createAndCloseFile(path.Join(projectDir, "mantis.yml"), diggerCfg)
	assert.NoError(t, err)
