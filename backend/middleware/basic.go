...
func HttpBasicApiAuth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// the next code block is required to use basic auth in case of APIS . Digger OOB looks out for non basic types i.e JWT etc
		// code block starts
		hasBasicHeader := strings.Contains(authHeader, "Basic ")
		if hasBasicHeader == true {
			log.Printf("Restricting access")
			username := os.Getenv("HTTP_BASIC_AUTH_USERNAME")
			password := os.Getenv("HTTP_BASIC_AUTH_PASSWORD")
			if username == "" || password == "" {
