...
func SetPRStatusForJobs(prService *github2.GithubService, prNumber int, jobs []orchestrator.Job) error {
	for _, job := range jobs {
		for _, command := range job.Commands {
			...
			case "mantis test":
				err = prService.SetStatus(prNumber, "pending", job.ProjectName+"/test")
			}
			if err != nil {
	...
	if len(jobs) > 0 {
		...
		if orchestrator.IsPlanJobs(jobs) {
			err = prService.SetStatus(prNumber, "pending", "mantis/plan")
		} else {
			err = prService.SetStatus(prNumber, "pending", "mantis/apply")
		}
		if err != nil {
		...
		err := prService.SetStatus(prNumber, "success", "mantis/plan")
		if err != nil {
			log.Printf("error setting status: %v", err)
			return fmt.Errorf("error setting pr status: %v", err)
		}
		err = prService.SetStatus(prNumber, "success", "mantis/apply")
		if err != nil {
			log.Printf("error setting status: %v", err)
			return fmt.Errorf("error setting pr status: %v", err)
		}
