...
type JobJson struct {
	...
	TestStage               StageJson         `json:"testStage"`
...
func (j *JobJson) IsPlan() bool {
	return slices.Contains(j.Commands, "mantis plan")
}

func (j *JobJson) IsApply() bool {
	return slices.Contains(j.Commands, "mantis apply")
}

func (j *JobJson) IsTest() bool {
	return slices.Contains(j.Commands, "mantis test")
}
...
func JobToJson(job Job, jobType DiggerCommand, organisationName string, branch string, commitSha string, jobToken string, backendHostname string, project digger_config.Project) JobJson {
	...
	return JobJson{
		...
		TestStage:               stageToJson(job.TestStage),
...
func JsonToJob(jobJson JobJson) Job {
	return Job{
		...
		TestStage:          jsonToStage(jobJson.TestStage),
...
func IsTestJobSpecs(jobs []JobJson) bool {
	isTest := true
	for _, job := range jobs {
		isTest = isTest && job.IsTest()
	}
	return isTest
}
